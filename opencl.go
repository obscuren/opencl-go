...
func New() (*EthashCL, error) {
	...
	dagBuff, err := context.CreateBufferUnsafe(cl.MemReadOnly, dagSize, nil)
	if err != nil {
		return nil, fmt.Errorf("dag buffer err:", err)
	}

	headerBuff, err := context.CreateBufferUnsafe(cl.MemReadOnly, 32, nil)
	if err != nil {
	...
	for i := 0; i < searchBuffSize; i++ {
		searchBuff, err := context.CreateBufferUnsafe(cl.MemReadOnly, maxSearchResults+1*SIZEOF_UINT32, nil)
		if err != nil {
